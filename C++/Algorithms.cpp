...
bool greedy_assignment(const vector<int> &user_file_size, const int &server_num, const int &limit, vector<int> &assign_res) {
    int server_idx = 0;
...
vector<int> decide_load_balancing(vector<int> user_file_size, const int &server_num) {
    ...
    int l = 0;
    int r = accumulate(user_file_size.cbegin(), user_file_size.cend(), 0);
    vector<int> feasible_assignment;
    while(l <= r) {
        int m = l + ((r-l) >> 1);
        vector<int> assign_res(server_num, 0);
        bool is_feasible = greedy_assignment(user_file_size, server_num, m, assign_res);
        if (is_feasible) {
